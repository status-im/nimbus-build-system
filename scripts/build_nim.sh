...
build_nim() {
	...
	if ! grep -q skipIntegrityCheck koch.nim; then
		...
		# - always use the latest version of Nimble and the tools
		. ci/funs.sh
		...
		if [[ "${QUICK_AND_DIRTY_COMPILER}" == "0" ]]; then
			# We want tools
			./koch --latest tools -d:release --skipUserCfg --skipParentCfg --warnings:off --hints:off
		elif [[ "${QUICK_AND_DIRTY_NIMBLE}" != "0" ]]; then
			# We just want nimble
			./koch --latest nimble -d:release --skipUserCfg --skipParentCfg --warnings:off --hints:off
		fi
	fi
