...
# script arguments
[[ $# -ne 4 ]] && {
  echo "Usage: $0 nim_dir csources_dir nimble_dir ci_cache_dir"
  exit 1
}
NIM_DIR="$1"
  ...
  if [[ -n "${NIM_COMMIT}" ]]; then
    ...
    if ! git checkout -q "${NIM_COMMIT}" 2>/dev/null; then
      ...
      git checkout -q "${NIM_COMMIT}" ||
        {
          echo "Error: wrong NIM_COMMIT specified:'${NIM_COMMIT}'"
          exit 1
        }
    fi
...
build_nim() {
  ...
  if [[ -n "${NIMBLE_COMMIT}" ]]; then
    ...
    git checkout -q "${NIMBLE_COMMIT}" || {
      echo "Error: wrong NIMBLE_COMMIT specified:'${NIMBLE_COMMIT}'"
      exit 1
    }
